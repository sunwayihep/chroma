...
namespace Chroma
{
	namespace MdagMSysSolverQUDAMULTIGRIDWilsonEnv
	{
		...
		{
			return new MdagMSysSolverQUDAMULTIGRIDWilson(A, state,SysSolverQUDAMULTIGRIDWilsonParams(xml_in, path));
		}
	...
	SystemSolverResults_t 
		MdagMSysSolverQUDAMULTIGRIDWilson::qudaInvert(const T& chi_s,
				T& psi_s) const{
			...
			void *spinorIn =(void *)&(mod_chi.elem(all.start()).elem(0).elem(0).real());
