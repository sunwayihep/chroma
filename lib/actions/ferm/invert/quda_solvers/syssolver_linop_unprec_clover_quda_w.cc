...
namespace Chroma
{
  namespace LinOpSysSolverQUDAUnprecCloverEnv
  {
    ...
    {
      //! Name to be used
      const std::string name("QUDA_UNPREC_CLOVER_INVERTER");
    ...
    {
      return new LinOpSysSolverQUDAUnprecClover(A, state,SysSolverQUDACloverParams(xml_in, path));
    }
  ...
  LinOpSysSolverQUDAUnprecClover::qudaInvert(const T& chi_s,
				       T& psi_s) const{
    ...
    QDPIO::cout << "QUDA_"<<solver_string<<"_UNPREC_CLOVER_SOLVER: time="<< quda_inv_param.secs <<" s" ;
